...
export default function EntitiesPage() {
  ...
  return <div className="mx-auto max-w-7xl animate-fade-up pb-12"><header className="border-b border-white/10 pb-8"><p className="eyebrow">Intelligence registry / canonical index</p><h1 className="title-serif mt-3 text-5xl">Entities</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-brand-gray-200">Canonical indicators and subjects discovered through authorized investigations. Open a dossier to inspect source-backed facts.</p></header>
    ...
    <div className="mt-5 grid gap-3 md:grid-cols-2">{items.map(item => <Link href={`/entities/${item.id}`} key={item.id} className="glass-panel group border-l-2 border-l-transparent p-5 transition hover:border-l-amber-300 hover:bg-black/10"><div className="flex items-start justify-between gap-4"><Badge value={item.kind}/><span className="text-xs text-brand-gray-200">Risk <b className="font-mono text-white">{item.riskScore.toFixed(1)}</b></span></div><p className="mt-5 truncate font-mono text-base text-white group-hover:text-amber-100">{item.value}</p><p className="mt-2 text-xs text-brand-gray-200">Last enriched {item.lastEnrichedAt ? new Date(item.lastEnrichedAt).toLocaleString() : 'not yet'}</p></Link>)}</div>
  </div>;
}
