...
export function Providers({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { const token = localStorage.getItem('osint.access-token'); setSession(token ? readSession(token) : null); if (token && !readSession(token)) localStorage.removeItem('osint.access-token'); setReady(true); }, []);
