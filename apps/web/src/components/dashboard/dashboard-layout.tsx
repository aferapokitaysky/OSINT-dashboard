...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#101722] text-[#f3eddf]">
      ...
      {/* Sidebar */}
      <aside className={cn('fixed left-0 top-0 bottom-0 z-40 w-72 border-r border-[#354256] bg-[#131c2a] flex flex-col transition-transform lg:w-64', mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0')}>
        <div className="border-b border-[#354256] p-6">
          <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center bg-[#62adff]">
            <ShieldAlert className="w-5 h-5 text-[#101722]" />
          </div>
          <span className="title-serif text-2xl">Ledger</span>
          </div>
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={cn(
                ...
                pathname === item.href
                  ? "border-[#62adff] bg-[#62adff]/10 text-white" 
        ...
        <div className="border-t border-[#354256] p-4">
      ...
      <main className="min-h-screen p-4 pt-20 lg:ml-64 lg:p-8">
        {/* Header */}
        <header className="mb-8 flex items-center justify-between border-b border-[#354256] pb-5">
          <div>
            <p className="eyebrow">Evidence ledger / workspace</p>
